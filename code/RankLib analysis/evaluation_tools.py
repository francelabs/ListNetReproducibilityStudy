...
def get_summary_metrics(lower_bound,
    ...
    """
    Returns a pandas dataframe containing the summary of performances across the different folds.
...
def write_metrics_to_csv(lower_bound,
    ...
    for fold in Folds:
        ...
        path = '../../'+data_fold+'/results_summary/Results_'+name+'_fold'+str(fold)+'_vali.csv'
        ...
        path = '../../'+data_fold+'/results_summary/Results_'+name+'_fold'+str(fold)+'_train.csv'
        ...
        if(return_test):
            path = '../../'+data_fold+'/results_summary/Results_'+name+'_fold'+str(fold)+'_test.csv'
    ...
    path = '../../'+data_fold+'/results_summary/Results_'+name+'_global_vali.csv'
    ...
    path = '../../'+data_fold+'/results_summary/Results_'+name+'_global_train.csv'
    ...
    if(return_test):
        path = '../../'+data_fold+'/results_summary/Results_'+name+'_global_test.csv'
...
def str_to_duration(duration_str):
    [minutes,secondes] = duration_str.split('m')
    minutes = float(minutes)
    secondes = float(secondes[:-1].replace(',','.'))
    duration = 60*minutes+secondes
    return duration


def get_training_duration(data_fold = "MQ2008", name = "RankLib",
                          Folds = [1,2,3,4,5]):
    duration = pd.DataFrame()

    for fold in Folds:
        col_duration_name = "duration_Fold"+str(fold)

        path = '../../'+data_fold+'/Fold'+str(fold)+'/models/'+name+'_training_duration.txt'
        data = pd.read_csv(path, sep=':', names=["model",col_duration_name])
        duration = pd.concat([duration,data[col_duration_name].apply(str_to_duration)],axis=1)

    duration_mean = duration.mean(axis=1)
